...
Diese Ausarbeitung \nocite{LeRi15} beschäftigt sich mit dem Spielkonzept von \textsc{Darwin's Avatars}. Der Spieler steuert eine \textit{evolutionäre virtuelle Kreatur}, im Folgenden EVK, mit der Tastatur und kann gegen eine KI oder einen weiteren menschlichen Spieler antreten. Das Ziel des Spiels ist, die Kreatur möglichst schnell durch eine Rennstrecke zu bewegen. Damit der Spieler stets für ihn unbekannte Kreaturen kontrollieren kann wird eine Methode verwendet, mit der neue Kreaturen erzeugt werden. Dies wird ermöglicht durch einen evolutionären Algorithmus. Die Vielfalt der daraus resultierenden Kreaturen ermöglicht einen langfristig motivierenden Spielspaß.
...
In Kapitel \ref{sec:evolution} wird die evolutionäre Generierung von Kreaturen erläutert.
Einige Ergebnisse dieses Verfahrens werden in Kapitel \ref{sec:beispiele} vorgestellt.
Kapitel \ref{sec:spiel} zeigt, wie die bisherigen Techniken zu einem Computerspiel zusammengesetzt werden.
Das Spiel wurde mit Probanden getestet.
Die Testergebnisse werden in Kapitel \ref{sec:ergebnisse} vorgestellt. Abschließend werden in Kapitel \ref{sec:betrachtungen} alternative und erweiterte Konzepte angeschnitten und in Kapitel \ref{sec:zusammenfassung} die Ergebnisse zusammengefasst.

\section{Kodierung}
\label{sec:kodierung}

Eine EVK benötigt einen Bauplan, der sie spezifiziert. Dieser heißt \textit{Genotyp}. Analog zu dem Begriff aus der Biologie speichert der Genotyp die Daten der Kreatur. 
...
Diese Kanten zeigen auf den Knoten von dem sie ausgehen.
...
\begin{figure}[H]
	...
	\caption{links: Genotyp mit Sensoren J0, J1 und Effektoren E0, E1 \cite{Sims94}; rechts: der zugehörige Phänotyp \label{fig:extendedgraph}}
\end{figure}
...
%----------------------------------------------------------------------------------
% Zu Igors Frage zu den anderen Eigenschaften eines GA habe ich nicht nachgebessert
%----------------------------------------------------------------------------------

EVK werden mithilfe eines evolutionären Algorithmus generiert.
Bei einem evolutionären Algorithmus werden nach dem Vorbild der Evolution in der Natur Entitäten, in diesem Fall EVK, bezüglich einer Fitnessfunktion optimiert.
...
Es werden sowohl Mutation als auch Rekombination zum Variieren der Lösungskandidaten verwendet.
Eine häufig verwendete Methode zur Rekombination ist die Kreuzung, die auch bei Darwin's Avatars zum Einsatz kommt.
Genetische Algorithmen wurden erstmalig von Holland vorgeschlagen \cite{Holland1992}.
...
Die Generierung von EVK wird bei Lessin, Fussel und Miikkulainen \cite{LeFuMi13} im Detail beschrieben und basiert auf der Arbeit von Sims \cite{Sims94}.
...
Es werden von beiden Elternteilen Teilketten gebildet, ein Präfix vom ersten Elternteil und ein Suffix vom zweiten.
...
\section{Beispiele}
\label{sec:beispiele}
Im Folgenden werden drei Beispiele von EVK dargestellt, die mit dem zuvor beschriebenen Verfahren entwickelt wurden.
...
\begin{figure}[H]
	...
	\end{subfigure}
	\caption{Drei evolutionäre virtuelle Kreaturen, nach \cite{LeRi15} \label{fig:beispiele}}
\end{figure}
...
Für jede der bereits vorgestellten Kreaturen wurden nun die erreichten Geschwindigkeiten der Spieler und der KI gemessen.
...
Die Diagramme in Abbildung \ref{fig:test1} zeigen den Grad der Aktivierung aller Muskeln in Abhängigkeit der Zeit.
Menschliche Spieler sind in grün dargestellt, das evolutionär generierte neuronale Netz ist in rot dargestellt.
Menschliche Spieler haben können Tasten nur entweder vollständig oder gar nicht betätigen.
Folglich sollten in den Diagrammen für Menschen nur weiße oder grüne Blöcke auftreten.
In der ursprünglichen Arbeit wird nicht erklärt, warum dennoch hellgrüne Bereiche zu sehen sind.

Die erreichte Geschwindigkeit findet sich oberhalb des Diagramms.
Die KI verwendete zur Fortbewegung eine simple Strategie. Sie benutzte lediglich die Muskeln 0 und 1 abwechselnd mit konstanter Frequenz. Damit erreichte sie die größte Geschwindigkeit. Die menschlichen Spieler nutzten eine ähnliche Strategie, jedoch weniger erfolgreich, da ihre Frequenz nicht konstant war. Die Spieler 5 und 6 verwendeten als einzige auch den Muskel 2. 
...
Die folgenden Diagramme fassen die Testergebnisse der einzelnen Kreaturen zusammen. Die Ergebnisse stammen aus 5 Durchläufen. Bei allen drei Beispielen verfolgte die KI über alle Durchläufe die gleiche Strategie. Das erklärt die konstanten Kurven im Diagramm. Ist die Kurve des besten Werts und des durchschnittlichen Werts der KI konstant, so folglich auch der Bereich der Standartabweichung. Das Chart zu Beispiel A \ref{fig:comp1} zeigt eine hohe Geschwindigkeit der KI von bis zu 2,5 Metern pro Sekunde. Die menschlichen Spieler konnten diese Geschwindigkeit auch nach mehreren Durchläufen nicht erreichen. Es zeigt sich aber in den ersten 3 Durchläufen eine Steigung der Ergebnisse. Nach Durchlauf 3 blieben sie aber konstant und fielen nach Durchlauf 4 sogar wieder ein Stück. Diese Kurve ist zu erwarten, denn zu Anfang weiß der menschliche Spieler nicht, wie er die Kreatur zu steuern hat. Er lernt es mit der Zeit jedoch, weswegen er nach mehreren Durchläufen bessere Ergebnisse erzielt. Die Ergebnisse zu Beispiel B \ref{fig:comp2} zeigen eine flachere Lernkurve der menschlichen Spieler. Hier wird deutlich, dass sowohl die KI, als auch die menschlichen Spieler eine geringere Geschwindigkeit als mit Kreatur A erreicht haben. Erneut tritt die Anomalie der fallenden Geschwindigkeitskurve im letzten Durchlauf auf. Dies könnte sich durch eine wechselnde Strategie der Spieler erklären, nachdem sie eine gewisse Sicherheit bei der Steuerung gewonnen haben. Das Vergleichschart zu Beispiel C \ref{fig:comp3} zeigt die interessantesten Kurven, denn hier gelangt die Spitze der Kurve mit den besten Werten der menschlichen Spieler in den Bereich der Standartabweichung der KI. Bei der KI fällt zudem auf, dass die beste Geschwindigkeit ein größeres Stück oberhalb der durchschnittlichen Werte liegt, als bei den anderen Beispielen. Auch die Kurven der menschlichen Spieler unterscheiden sich gegenüber denen von vorherigen Beispielen. So wurde in Durchlauf 2 ein lokales Maximum beim besten Wert erreicht, dadurch aber die Streuung in der Standartabweichung erhöht. In Durchlauf 3 konnten keine so hohen Geschwindigkeiten von den menschlichen Spielern erzielt werden, doch mit den beiden folgenden Durchläufen haben sich die Werte wieder gebessert. 

\begin{figure}	
	\centering
	\includegraphics[width=0.8\textwidth]{img/Comparison_A.png}
	\caption{Vergleichschart zu Beispiel A \cite{LeRi15}\label{fig:comp1}}
\end{figure}
\begin{figure}
	\centering
	\includegraphics[width=0.8\textwidth]{img/Comparison_B.png}
	\caption{Vergleichschart zu Beispiel B \cite{LeRi15}\label{fig:comp2}}		
\end{figure}
\begin{figure}
	\centering
	\includegraphics[width=0.8\textwidth]{img/Comparison_C.png}
	\caption{Vergleichschart zu Beispiel C \cite{LeRi15}\label{fig:comp3}}	
\end{figure}

\section{Weiterführende Betrachtungen}
\label{sec:betrachtungen}
...
Die Testergebnisse in Kapitel \ref{sec:ergebnisse} zeigen, dass sich so ein tragfähiges Spielkonzept ergibt und die EVK interessante Spielgegenstände darstellen.
