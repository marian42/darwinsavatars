...
Diese Ausarbeitung \nocite{LeRi15} beschäftigt sich mit dem Spielkonzept von \textsc{Darwin's Avatars}. Der Spieler steuert eine prozedural generierte Kreatur mit der Tastatur und kann gegen eine KI oder einen weiteren menschlichen Spieler antreten. Das Ziel des Spiels ist, die Kreatur möglichst schnell durch eine Rennstrecke zu bewegen. Damit der Spieler stets für ihn unbekannte Kreaturen kontrollieren kann wird eine Methode verwendet, mit der neue Kreaturen erzeugt werden. Dies wird ermöglicht durch einen evolutionären Algorithmus. Durch die Vielfalt der daraus resultierenden Kreaturen entsteht ein langfristig motivierender Spielspaß.
...
Der evolutionäre Algorithmus generiert ein neuronales Netz welches eine KI Steuerung der Kreatur ermöglicht. Dieser Algorithmus entstammt einer früheren Arbeit von Lessin, Fussel und Miikkulainen \cite{LeFuMi13}. Er berechnet für jede Kreatur einen Fitnesswert, der ihre Bewegungsgeschwindigkeit bewertet. Auch die Ästhetik jener Kreaturen wird dabei berücksichtigt. Über mehrere hundert Generationen werden die Kreaturen mit dem höchsten Fitnesswert selektiert und durch Paarung oder Mutation neue Kreaturen generiert.



\section{Kodierung}

Die evolutionären virtuellen Kreaturen, im Folgenden EVKs, benötigen einen Bauplan der sie spezifiziert. Dieser heißt \textit{Genotyp}. Analog zu dem Begriff aus der Biologie speichert der Genotyp die Daten der Kreatur. Dazu gehören neben Struktur und Muskeln auch komplexere Informationen wie die Art der Verbindung zweier Segmente zueinander. Wie bei den Lebewesen brauchen die Knochen, in dem Fall die Körpersegmente, Gelenke um eine bestimmte Bewegungsart auszuführen. Es eignen sich dafür verschiedene Gelenktypen. So können die Gelenke starr, beugend, verdreht, oder kugelförmig ausfallen. Der Parameter \textit{Gelenkfreiheit} bestimmt den Grad bis zu dem sich das Gelenk bewegen lässt. Die Körpersegmente werden mit Muskeln bespannt, um Bewegungen auszulösen. Gespeichert werden die Daten in einem Graphen wie in Abbildung \ref{fig:genotyp}. Gleich unter dem jeweiligen Graphen ist die zugehörige Kreatur abgebildet. Sie wird durch den \textit{Phänotypen} beschrieben. Der Phänotyp beschreibt die virtuelle Repräsentation der Kreatur.

Die Graphen (a) und (b) führen zur gleichen Kreatur. Das erklärt sich dadurch, dass Graph (a) redundante Informationen enthält. Die Kreatur besteht zwar aus drei Körpersegmenten, aber nur aus zwei verschiedenen Körpersegmenttypen. Es genügt zu speichern, dass das eine Segment mit zwei anderen gleichartigen Segmenten verbunden ist. Die Graphen (c) und (d) formen komplexere Kreaturen. Dazu wird die Eigenschaft reflexiver Kanten ausgenutzt. Diese Kanten bilden auf ein gleichartiges Körpersegment ab. Dadurch entstehen Ketten des gleichen Segments. Oft ist das eine gewünschte Eigenschaft wie bei den Flügeln der Kreatur (c), oder den Gliedmaßen von Figur (d). Die reflexiven Kanten können beliebig oft verfolgt werden. Damit ist eine endlose Rekursion möglich. Da unendlich große Kreaturen am Computer nicht dargestellt werden können wird durch das Setzen eines \textit{rekursiven Limits} die Anzahl der Körpersegmente beschränkt. 
...
Das Ergebnis der Kodierung ist eine Kreatur wie sie im Spiel gesteuert werden kann. Der Genotyp wird jetzt um ein neuronales Netz erweitert. Das neuronale Netz steuert die Kreatur. Dafür werden mehrere Komponenten benötigt, die im Folgenden erklärt werden. 

\subsection{Neuronen}

\textit{Neuronen} wenden arithmetische oder logische Operationen auf ein Eingangssignal an und geben es anschließend an ihrem Ausgang zurück. Im Graphen werden die Neuronen als Knoten dargestellt. Sie arbeiten dabei im Wertebereich $[0,1]$. Mögliche Funktionen sind:
...
\begin{tabular}{l l l l}
	...
	Glätten & Speichern & Oszillierend & Sägeförmig oszillierend
\end{tabular}
\vspace{0.7em}

Wie die Neuronen mit konkreten Werten arbeiten ist in Abbildung \ref{fig:neuronen} ersichtlich. Der Sinus errechnet einen vom Zeitparameter $t$ abhängigen Wert aus der Sinusfunktion. Das Komplement dreht den eingehenden Wert um. Aus $x$ wird $1 - x$. Dieser Parameter wird allerdings nicht als Eingangssignal gegeben, er kommt vom System.
...
\begin{figure}[H]
	...
	\caption{Genotyp mit Sensoren J0, J1 und Effektoren E0, E1 und der zugehörige Phänotyp \label{fig:extendedgraph}}
\end{figure}
...
\begin{figure}[H]
	...
	\end{subfigure}
	\caption{Drei evolutionäre virtuelle Kreaturen \label{fig:beispiele}}
\end{figure}
...
\begin{figure}[H]
	...
	\end{subfigure}
	\caption{Testergebnisse für Beispiel A \label{fig:test1}}
\end{figure}
...
\begin{figure}[H]
	...
	\end{subfigure}
	\caption{Testergebnisse für Beispiel B \label{fig:test2}}
\end{figure}
...
\begin{figure}[H]
	...
	\end{subfigure}
	\caption{Testergebnisse für Beispiel C \label{fig:test3}}
\end{figure}
...
\begin{figure}[H]
		...
		\centering
		\includegraphics[width=0.7\textwidth]{img/Comparison_A.png}
		\caption{Vergleichschart zu Beispiel A \label{fig:comp1}}		
	
\end{figure}
\begin{figure}[H]	
		\centering
		\includegraphics[width=0.7\textwidth]{img/Comparison_B.png}
		\caption{Vergleichschart zu Beispiel B \label{fig:comp2}}		
\end{figure}
\begin{figure}[H]
	\centering
	\includegraphics[width=0.7\textwidth]{img/Comparison_C.png}
	\caption{Vergleichschart zu Beispiel C \label{fig:comp3}}	
\end{figure}
